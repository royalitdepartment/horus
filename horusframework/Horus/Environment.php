 ...
 * be common across multiple Horus applications.
 *
...
class Environment implements \ArrayAccess, \IteratorAggregate
{
     ...
     * variables by using `\Horus\Environment::mock()` instead.
     *
    ...
    public static function mock($userSettings = array())
    {
        $defaults = array(
            ...
            'USER_AGENT' => 'Horus Framework',
            'REMOTE_ADDR' => '127.0.0.1',
            'Horus.url_scheme' => 'http',
            'Horus.input' => '',
            'Horus.errors' => @fopen('php://stderr', 'w')
        );
    ...
    private function __construct($settings = null)
    {
        if ($settings) {
            ...
            $headers = \Horus\Http\Headers::extract($_SERVER);
            foreach ($headers as $key => $value) {
                $env[$key] = $value;
            }

            //Is the application running under HTTPS or HTTP protocol?
            $env['Horus.url_scheme'] = empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] === 'off' ? 'http' : 'https';
            ...
            $env['Horus.input'] = $rawInput;

            //Error stream
            $env['Horus.errors'] = @fopen('php://stderr', 'w');
