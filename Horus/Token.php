 ...
 */
class Token extends \Horus\Helpers\Encryption {
	protected $token;
	...
	public function generateToken($token_data, $expiry_time = (7 * 24 * 60 * 60)) {
		...
		$payload["secret_phrase"] = "@Horus1sGr8";
	...
	public function validateToken($token) {
		// check if the app secret is @Horus1sGr8
		$token = $this->decrypt($token, $this->key, $this->nonce);
		$token = json_decode($token);
		if ($token['secret_phrase' != "@Horus1sGr8"] || !isset($token['secret_phrase'])) {
